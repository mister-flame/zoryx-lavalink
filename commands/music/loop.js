...
module.exports = {
    ...
    async execute(interaction) {
        ...
        if (!choice || (choice !== "track" && choice !== "queue" && choice !== "off")) {
            return interaction.editReply({ content: `❌ Précise "track", "queue" ou "off". (Actuellement : \`${player.repeatMode}\`)` }).then(() => {
        ...
        return interaction.editReply({ content: `${emojiRepeat} Mode boucle définit sur le mode : \`${player.repeatMode}\`.` }).then(() => {
            setTimeout(() => interaction.deleteReply().catch(() => { }), 15000);
        }).catch(() => { });
    },
};